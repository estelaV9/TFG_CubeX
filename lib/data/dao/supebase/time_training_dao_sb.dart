...
///
/// Esta clase interactúa con la base de datos de Supabase para insertar, eliminar, obtener y
/// calcular estadísticas relacionadas con los tiempos en las sesiones.
class TimeTrainingDaoSb {
  ...
  Future<String> getPbTimeBySession(
    ...
    if (isDnf) {
      ...
      for (var times in timesList) {
        ...
        if (times.timeInSeconds < pbTime && times.penalty != "DNF") {
  ...
  Future<bool> deleteTime(int idTimeTraining) async {
    try {
      ...
      final deleteTime = await supabase
          .from('timetraining')
          .delete()
          .eq('idtimetraining', idTimeTraining)
          .select();

      // DEVUELVE TRUE/FALSE SI SE ELIMINO CORRECTAMENTE O NO
      return deleteTime.isNotEmpty;
    } catch (e) {
  ...
  Future<bool> deleteAllTimeBySession(int? idSession) async {
    try {
      ...
      final deleteAllTime = await supabase
          .from('timetraining')
          .delete()
          .eq('idsession', idSession!)
          .select();

      // DEVUELVE TRUE/FALSE SI SE ELIMINARON CORRECTAMENTE O NO
      return deleteAllTime.isNotEmpty;
    } catch (e) {
      // RETORNA FALSE Y UN MENSAJE SI OCURRE UN ERROR
      DatabaseHelper.logger
          .e("Error al eliminar todos los tiempos de la sesión $idSession: $e");
      return false;
    }
  ...
  Future<TimeTraining?> getTimeById(int idTime) async {
    try {
      ...
      if (result.isNotEmpty) {
        ...
        // SI NO ENCUENTRA EL TIEMPO, DEVUELVE UN -1
        DatabaseHelper.logger
            .w("No se encontró ningún tiempo con ese id: $idTime");
      } // SI LA CONSULTA NO ES NULA Y DEVUELVE -1
